...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
            ...
            IUserAppContext userAppContext)
        {
            ...
            _userExperienceRepository= experienceRepository;
        ...
        public async Task<IActionResult> GetLanguages()
        {
            ...
            var lang = await _userLanguageRepository.FindAsync(x=>x.UserId == talentId);
        ...
        {
            //Your code here;
           UserLanguage ul= (await _userLanguageRepository.GetByIdAsync(language.Id));

            ul.LanguageLevel = language.Level;
            ul.Language = language.Name;
           await _userLanguageRepository.Update(ul);

            return Json(new { Success = true, data = ul });
        ...
        public async Task<IActionResult> GetSkills()
        {
            ...
            String talentId = _userAppContext.CurrentUserId;
            var sk = await _userSkillRepository.FindAsync(x => x.UserId == talentId);
           
            return Json(new { Success = true, data = sk }); ;
        }
        ...
        {
            //Your code here;
            UserSkill sk = (await _userSkillRepository.GetByIdAsync(skill.Id));

            sk.Skill = skill.Name;
            sk.ExperienceLevel= skill.Level;
            
            await _userSkillRepository.Update(sk);
            return Json(new { Success = true, data = sk }); ;
        ...
        [HttpPost("addExp")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public ActionResult AddExp([FromBody] ExperienceViewModel exp)
        {
            //Your code here;
            // throw new NotImplementedException();
            var ex = new UserExperience()
            {
                Id = exp.Id,
                UserId = _userAppContext.CurrentUserId,
                Company = exp.Company,
            ...
            _userExperienceRepository.Add(ex);
        ...
        public async Task<IActionResult> GetExp()
        {
            ...
            // throw new NotImplementedException();
            String talentId = _userAppContext.CurrentUserId;
            var ex = await _userExperienceRepository.FindAsync(x => x.UserId == talentId);
        ...
        [HttpPost("deleteExp")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<IActionResult> DeleteExp([FromBody] ExperienceViewModel exp)
        {
            //Your code here;
            UserExperience ex = (await _userExperienceRepository.GetByIdAsync(exp.Id));

            await _userExperienceRepository.Delete(ex);
        ...
        [HttpPost("updateExp")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<IActionResult> UpdateExp([FromBody] ExperienceViewModel exp)
        {
            //Your code here;
            UserExperience ex = (await _userExperienceRepository.GetByIdAsync(exp.Id));
            ...
            ex.Responsibilities = exp.Responsibilities;
            ex.Start=exp.Start;
            ex.End=exp.End;

            await _userExperienceRepository.Update(ex);
            return Json(new { Success = true, data = ex }); ;
        ...
        public async Task<ActionResult> UpdateProfilePhoto()
        {
            ...
            try
            {
                ...
                await _profileService.UpdateTalentPhoto(id, file);

                return Json(new { Success = true,data = "Image Uploaded Successfully" + file});
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> UpdateTalentProfile([FromBody]TalentProfileViewModel profile)
        {
            if (ModelState.IsValid)
            {
                ...
                //var userProfile = await _profileService.UpdateTalentProfile(profile, _userAppContext.CurrentUserId);
                //    if (userProfile)
                //    {
                //    return Json(new { Success = true,data=userProfile });
                //    }
        ...
        public async Task<IActionResult> GetTalentSnapshots(FeedIncrementModel feed)
        {
            try
            {
                ...
                var usr = _userRepository.Collection;
                //var usr1 = usr.Select(x=>x.Id).ToList();
               var sk = _userSkillRepository.GetQueryable().Select(x => x.Skill).Take(5).ToList();
              // var sk = _userSkillRepository.Collection;
                // var sk = _userSkillRepository.GetQueryable().Where(x=>x.UserId==" ").ToList();
               
                var exp = _userExperienceRepository.Collection;

                var tlnt = (from u in usr.AsQueryable()
                            join x in exp.AsQueryable()                            
                            on u.Id equals x.UserId
                            //join s in sk.AsQueryable()
                            ...
                            select new TalentSnapshotViewModel
                            {
                                ...
                                PhotoId = u.ProfilePhoto,
                                Visa = u.VisaStatus,
                                CurrentEmployment = x.Company,
                                Level = x.Position,
                                Skills = sk //new List<string> { s.Skill }
                            }).ToList();
                var result = tlnt.GroupBy(x => x.Id).Select(x=>x.FirstOrDefault());
